...
class SimulationDataEcoli:
    ...
    def _addConditionData(self, raw_data):
        self.conditionToDoublingTime = dict([(x["condition"], x["doubling time"]) for x in raw_data.condition.condition_defs])
        self.conditionActiveTfs = dict([(x["condition"], x["active TFs"]) for x in raw_data.condition.condition_defs])

        abbrToActiveId = dict([(x["TF"], x["activeId"].split(", ")) for x in raw_data.tfIds if len(x["activeId"]) > 0])

        geneIdToRnaId = dict([(x["id"], x["rnaId"]) for x in raw_data.genes])

        abbrToRnaId = dict(
            [(x["symbol"], x["rnaId"]) for x in raw_data.genes] +
            [(x["name"], geneIdToRnaId[x["geneId"]]) for x in raw_data.translationEfficiency if x["geneId"] != "#N/A"]
            )
